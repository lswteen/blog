...
1) Query String으로 구성된 패턴
   https://jobkorea.co.kr/list?page=1
   https://jobkorea.co.kr/keyword?abc

2) Path Variable
   https://jobkorea.co.kr/list/1
   https://jobkorea.co.kr/keyword/abc
...
https://www.jobkorea.co.kr/OnePick/Serviceinfo
https://www.jobkorea.co.kr/onepick/joblist?Page=1&PageSize=20&Ord=GcmCodeAmtDesc
...
서브도메인: 예) news.performars.com



서브 도메인은 각각의 도메인이므로 새로운 토픽을 관리할 때 사용한다
SEO 점수가 호스트 도메인에 흡수가 되지 않는다는 견해도 존재한다
Call to action  있는 랜딩 페이지 하나만 관리하고 싶으면 서브도메인이 편하다 ( 예:  Hubspot)
니치 마켓을 타깃 할 때 좋다
관리가 불편하다
큰 회사일수록 서브도메인 사용이 좀 더 효율성 있다




서브폴더: 예) performars.com/news

서브 폴더는 여러 서비스들을 호스트 도메인 아래로 연결 시켜서 관리한다.
여러 서브 폴더는 하나의 호스트 도메인에 속해 있는 SEO 점수가 루트 도메인으로 흡수된다고 한다
서브 폴더의 내용은 주로 도메인과 연관성이 강하게 있으면 좋다
관리가 편하다
SEO ranking 을 위해서는 서브폴더가 유리할 수 있다


랭킹상승목적이라면 서브폴더를 추천드립니다.
